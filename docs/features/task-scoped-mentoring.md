...
> **本改訂（2026-09-22）は判断7 を「未決の論点」から決定へ移し（決定 20）、S3 の形を「メンタリング記録から当該会話へ辿る読み取り専用の導線」に確定して（決定 21・22）、実装対象を S3 にする。** S2b（#545 / PR #549・`96d2e9b`）と S2a（#557 / PR #559・`dddba97`）は出荷済みで、本改訂に入る前に Issue #438 で残っていたのは判断 6「綿密に相談した**やり取り**をあとから読み返せるか」と判断 7「朝会ゲートとの関係」の 2 点であり、本改訂はその両方を閉じる（判断 7 → 決定 20、判断 6 → 決定 21・22）。「### S3 の確証」節・決定 20〜22・受入基準の「### メンタリング記録から当該会話へ辿る導線（S3）」節を本改訂で追加した。
>
> 過去の改訂: S2（2026-09-20・決定 13〜19。S2 を S2a / S2b に割り、S2b → S2a の順で出荷）／S1b（Issue #476・2026-09-12・決定 10・11・12）／S1a（Issue #474・要件チケット #489・2026-09-11・決定 8・9）。いずれも出荷済みで、S1 の受入基準は覆っていない（S1 は `73c1b20`）。
...
- **(E) タスク起点の導線を `adhoc` 区間に限れば、朝会ゲートには一切算入されない。** ゲートの件数は `session_id` スコープであり（`sessions-routes.ts:144`）、`record_mentoring` に渡る `sessionId` はチャットルートが渡す現在セッションである（`chat-messages-route.ts:325-326`）。`adhoc` セッションに記録された mentoring 行は朝会セッションの件数に入らないため、**ゲートのコードを 1 行も変えずに現状の判定が保たれる**。本スライスの受入基準が朝会ゲートとの関係（判断7。2026-09-22 に決定 20 で「現状維持」と確定）から独立できる根拠はここにある。
...
### S3 の確証（2026-09-22 時点の実コード）

S3（メンタリング記録から当該会話へ辿る導線）の前提を実コードで裏取りした結果を記録する。Issue #438 のコメント（2026-09-22・判断7 の決定）と計画承認時の記述を対照し、**食い違った点は判定欄に「補正」と書いた。**

| # | 計画時の記述・観察 | 実コードの実態 | 判定 |
|---|---|---|---|
| 1 | メンタリング記録は `session_id` を持つ | `decisions.session_id` は `INTEGER NOT NULL REFERENCES sessions(id)`（`migrate.ts:153-154`）。`insertDecision`（`decisions-repository.ts:45`）は `session_id` を必須で受け取り、`executeRecordMentoringTool`（`mentoring-tool.ts:89` で `kind: "mentoring"`）へ渡る `sessionId` は `POST /api/sessions/:id/messages` のパスの `id`（`chat-messages-route.ts:186-188`・`:431`）＝**そのメンタリングを行ったセッション**である。**`session_id` を持たない記録は存在しない**（列が NOT NULL のため、旧データも含めて空になりえない） | 一致 |
| 2 | 記録はセッション内のどの発言から始まったかを持つ | **持たない**。`decisions` に `message_id` 相当の列は無く、あるのは `created_at` だけ。発言単位の位置付けは `created_at` と `messages.created_at` の比較による近似にしかならない | **補正**（決定 22 で導線の粒度をセッション単位に固定した） |
| 3 | web から過去セッションの会話を開ける | **開けない**。セッションに触る web ファイルは `use-chat.ts` と `chat-api.ts` の 2 つだけで、タイムラインは `selectTimelineSessions`（`merge-timeline.ts:30-38`）が**当日（ローカル暦日）のセッション＋活性セッション**に絞る。セッション一覧・日付ナビゲーション・過去会話のビューは存在しない | **補正**（決定 21 で読み取り専用の面を新設する。S3 の規模は S ではなく M） |
| 4 | 会話を読むための API がある | `GET /api/sessions`（`sessions-routes.ts:181`）と `GET /api/sessions/:id/messages`（同 `:326`）が任意のセッションを返す。web 側の呼び出し関数 `fetchSessionMessages`（`chat-api.ts:100-108`）も既にあり、`!response.ok` で `ChatApiError`（`code` 透過）を投げる。**サーバ変更なしで到達可能** | 一致 |
| 5 | `kind='mentoring'` の記録は決定ログに表示される | `DecisionLog.tsx:13-46` の `DecisionCard` が「メンタリング」ラベル・日時・`content`・`rationale` を描画し、決定と同じタスク別セクション内で時系列混在（#358 判断3）。web の `DecisionRecord` は `session_id` を既に持つ（`decision.ts:18`）ため、**導線の追加に API 変更は要らない** | 一致 |
| 6 | 過去セッションが消えると導線が dangling になる | `sessions` を削除・失効させる機構は本番コードに無い（`session-not-found-response-shape.md` 確証 (F) と同じ）。`messages` の削除はやりなおし（`deleteMessagesFrom`・`messages-repository.ts:108`）だけで、消えるのは差し替え起点以降の発言のみ。**セッション単位の導線が指す先は消えない**が、やりなおされたターンのやり取りは消えていることがある | 一致（決定 22 で「残っている会話をそのまま表示する」とした） |

#### 設計に影響する実態（T〜V）

- **(T) タイムラインは活性セッション宛の送信欄と一体である。** `useChat.send` は常に `sessionIdRef.current`（活性セッション）へ送る（`use-chat.ts:477-503`）。過去セッションを既存タイムラインへ pin して表示すると（`selectTimelineSessions` の `pinnedSessionIds` は日付を跨ぐ活性セッションのための機構）、**過去の会話を表示しながら入力欄は当日へ送る**状態になり、pin の解除・リロード時の復元など S1b と同種の状態管理を再び持ち込む（決定 21 が (A1) を却下する根拠）。
- **(U) `ChatView` は `chatState` に結合している。** `ChatView` は `useChat` の状態（送信・切替・編集・相談中）を前提に描画するため、過去セッションの読み取り専用表示に流用すると送信欄・ヘッダ導線・相談中の帯を無効化する分岐が要る。読み取り専用の面は**別コンポーネント**として `ChatMessage[]` を描画するほうが、S1b の状態（`mentoringTarget`）と送信経路に触れないことを構造的に保てる。
- **(V) 1 つのセッションを複数の記録が指しうる。** `adhoc` セッションはローカル暦日のほぼ全体に渡り（確証 (H)）、1 日に複数タスクのメンタリングを行えば、それぞれの記録が同じ `session_id` を持つ。会話だけを表示すると、どの記録から開いたかが区別できない（決定 22 が「開いた記録の対象タスクと日時を表示する」を要件にする根拠）。
...
> **本改訂（S3）の対象は末尾の「（S3）」を付した 6 項目のみ**。`- [x]` は S1（`73c1b20`）・S1a（#489・`75794aa`）・S1b（#476・`4106645`）・S2b（#545・`96d2e9b`）・S2a（#557・`dddba97`）で**出荷済み**である。
...
- [x] タスク起点のメンタリングで、ボスが対象タスクに紐づく過去の決定・過去のメンタリング記録を踏まえて深掘りできる（S2b）
- [x] 過去記録の投入によって、既存の「直近の決定」枠（`kind='mentoring'` を除外する #408 AC-42 の契約）が変わらない（S2b）
- [x] 対象タスクに過去の記録が無いときは、ボスのシステムプロンプトに空の記録セクションが積まれない（S2b）
- [x] タスクカードから、そのタスクについて過去に何を相談し何を決めたかを決定ログで読み返せる（S2a）
- [x] その導線をたどると、決定ログの中の対象タスクのセクションが見える位置に表示される（S2a）
- [ ] 決定ログのメンタリング記録から、その記録が生まれたセッションの会話（やり取り）を読み返せる（S3）
- [ ] 会話を読み返す面は読み取り専用で、そこから発言を送ることはできない（S3）
- [ ] 会話を読み返す面には、開いた記録の対象タスクのタイトル（`task_id` を持たない記録では決定ログの同じセクションの見出し）が表示される（S3）
- [ ] 会話を読み返す面には、開いた記録の日時が表示される（S3）
- [ ] 会話を読み返す面を開いている間、「相談中」の状態（`mentoringTarget`）は変わらない（S3）
- [ ] 会話を読み返す面を開いている間、活性セッションへの送信・セッションの作成・会の終了は発生しない（S3）
...
- **変更対象（S2b・出荷済み）**: `server/src/decisions/decisions-repository.ts`（新規クエリ関数）/ `server/src/sessions/chat-messages-route.ts`（結線）/ `server/src/boss/persona-prompt.ts`（セクションの描画）とそれぞれのテスト。**web 側は 1 行も触らない・マイグレーションもしない**（計 6 ファイル）
- **変更対象（S2a・出荷済み）**: `web/src/TaskCard.tsx` / `web/src/TaskBoard.tsx` / `web/src/AppLayout.tsx` / `web/src/DecisionLog.tsx` とそれぞれのテスト。**サーバ側は 1 行も触らない**（計 8 ファイル）。S2a と S2b は触るファイルが 1 つも重ならず、S2b → S2a の順で出荷した（決定 13）
- **変更対象（S3・本改訂の実装対象）**: `web/src/DecisionLog.tsx`（メンタリング記録カードの導線）/ 読み取り専用の会話面の**新規コンポーネント**（既存の `fetchSessionMessages` で取得した `ChatMessage[]` を描画する）/ `web/src/AppLayout.tsx`（面の開閉の state を `DecisionLog` の外に置く場合のみ）とそれぞれのテスト。`web/src/chat-api.ts` は既存関数の再利用のみで変更不要の見込み。**サーバ側は 1 行も触らない・マイグレーションもしない**（web 4-5 ファイル＋テスト 4-5＝ M 相当。決定 21）
  - **`use-chat.ts` / `ChatView.tsx` は S3 で変更しない**（確証 (T)・(U)。読み取り専用の面は送信経路と `mentoringTarget` に触れない）
...
### 画面（メンタリング記録カードの導線・S3）

決定ログのタスク別セクション内の **`kind='mentoring'` の記録カード**（`DecisionCard`）に、その記録が生まれたセッションの会話を読む導線を 1 つ追加する。`kind='decision'` のカードには置かない（決定 22）。

- **表示条件**: 常時（メンタリング記録はすべて `session_id` を持つ。S3 の確証 1）。**`task_id` を持たない記録（朝会・全日単位メンタリングの結論。「タスクに紐づかない決定」セクションに並ぶ）にも出す**（決定 22）。会（朝会・夕会）の最中にも出す（読むだけで、発言も記録も朝会ゲートも触らない。決定 14 と同じ規律）
- **押したときの挙動**: そのセッションの会話が読み取り専用の面に表示される（決定 21）
- 既存のカードの描画内容（種別ラベル・日時・`content`・`rationale`）は変えない

### 画面（読み取り専用の会話面・S3）

メンタリング記録の導線から開く、**1 セッション分の会話を読むだけの面**。モーダルにするかビューにするかは実装側の裁量とし、仕様では次だけを要件にする。

- **読み取り専用**: 入力欄・送信ボタン・やりなおし・会の開始／終了・メンタリングの開始導線を持たない
- **出自の表示**: どの記録から開いたかとして、**その記録が決定ログで属するセクションの見出しと同じ文言**（`task_id` を持つ記録は対象タスクのタイトル `task_title`〔確証 (L) のとおりタスクは削除できないので `null` にならないが、`groupDecisionsByTask` と同じく `#<task_id>` を防御的フォールバックにする〕、`task_id` を持たない記録は `UNASSIGNED_SECTION_TITLE`「タスクに紐づかない決定」）と**記録の日時**（`created_at`）を表示する（確証 (V)・決定 22）
- **会話の表示**: `GET /api/sessions/:id/messages` の結果を、利用者の発言とボスの発言が区別できる形で古い順に表示する。`interrupted` の扱い・日時の書式は既存の `ChatView` と揃えることが望ましいが、受入基準では固定しない
- **取得中・失敗の表示**: 取得中はその旨を表示し、404（`code: "session_not_found"`）とそれ以外の失敗は区別して表示する（既存の応答形は `docs/features/session-not-found-response-shape.md`）
- **閉じる導線**: 面を閉じると元の決定ログの表示に戻る
- **活性セッションに触れない**: この面の開閉と取得は `useChat` の状態（活性セッション・`mentoringTarget`・送信中）を読みも書きもしない
...
### IF（S3・メンタリング記録から当該会話へ辿る導線）

```text
DecisionLog / DecisionCard
    kind === "mentoring" のカードにだけ導線を描画し、押されたら
    { sessionId, taskTitle, recordedAt } 相当を「開いている記録」として持つ
    （state の持ち主は DecisionLog でも AppLayout でもよい。仮置き）
   ↓ props
読み取り専用の会話面（新規コンポーネント）
    chat-api.fetchSessionMessages(sessionId) で取得し、ChatMessage[] を
    古い順に描画する。失敗は ChatApiError の code で 404 とそれ以外を
    出し分ける。送信・やりなおし・セッション操作の手段を持たない。
    閉じる操作で「開いている記録」を null に戻す
   ↓ GET /api/sessions/:id/messages（既存・変更なし）
```

> コンポーネント名・prop 名・state の置き場所は仮置きである。**面が `useChat` の状態に依存しないこと**（決定 22）と**導線が `kind='mentoring'` のカードにだけ付くこと**（決定 22）は固定する。
...
> Issue #438 の要決定点に対する決定。後続の実装はこの決定に従い、独自判断で逸脱しない。判断7（朝会ゲートとの関係）は決定 20 で決定済み（2026-09-22）。
>
> **見出しの番号は本仕様での通し番号であり、Issue #438 の「判断 N」とは別体系である。** 対応がある決定には見出しに `（判断N）` を併記した。併記の無い見出し（4・6・7・8・9・10・11・12・13・16・17・18・19）は本仕様での追加決定であり、**とくに見出し 7 は Issue の「判断7」とは無関係**である（判断7 に対応するのは決定 20）。決定 8・9 は S1a（Issue #474）、決定 10・11・12 は S1b（Issue #476）、**決定 13〜19 は S2（13 はスライスの切り方、14・15 は S2a、16〜19 は S2b）**、**決定 20〜22 は S3（20 は判断7、21・22 は判断6）**の決定で、いずれも意思決定者の承認済みである。
...
### 20. 朝会の必須メンタリングゲートとの関係（S3・Issue #438 判断7）

> オーナー（人間）が 2026-09-22 に Issue #438 のコメントで決定した方針であり、実装はこの決定に従う。

- **採用案**: **(1) 満たさない（現状維持）**。タスク起点のメンタリングは朝会の必須メンタリングゲートを満たさない。タスク起点導線は引き続き `adhoc` 区間のみに出す（決定 1）。`mentoring-gate.ts` は変えない
- **理由**: 朝会のゲート（`MENTORING_FLOW_INSTRUCTION`）は「今日の仕事の進め方」＝全体を通した軽い調整を担保するもので、タスクごとの深掘り（タスクカード「メンタリングする」）とは役割が別。朝会の時点で全タスクの深掘りを求めず、逆にタスク 1 件の深掘りで全体の調整を代替させない
- **代替案**: (2) 朝会中にもタスク起点導線を出し、その記録でゲートを満たす — 却下（朝会がタスク 1 件の相談で終わりうる＝規律が緩む）。(3) 朝会中は導線を出すがゲート判定は全日メンタリング記録に限る — 却下（判定側に「タスク紐づきの有無」を持ち込み、`mentoring-gate.ts` の変更が発生する）
- **判断材料（実コード）**: ゲートは `isMentoringComplete({ mentoringRecordCount, userMessageCount })` の AND（`mentoring-gate.ts:25`）で、件数は朝会セッションの `session_id` スコープ（`sessions-routes.ts:144`）。設定 `morning_mentoring_required`（既定 true・`server/src/settings/mentoring-settings.ts:21`）でゲート自体をオフにできる。`adhoc` 中のメンタリングは朝会セッションに属さないため**今もゲートを満たしておらず**、(1) はコード変更ゼロである
- **オーナーの問題意識（タスクごとの見積もり・進め方のずれが全体に波及する）への応答は本決定ではなく「タスクごとのメンタリングをいつ促すか」で行う。** これは別 Issue #561 の範囲であり、本仕様では扱わない（参照のみ）
- **影響範囲**: コード変更ゼロ。本仕様の「未決の論点」から判断7 を取り除き、S3 の範囲から「朝会ゲートとの関係」を外す

### 21. S3 の形——メンタリング記録から当該会話へ辿る読み取り専用の導線（S3・Issue #438 判断6）

> オーナー（人間）が 2026-09-22 の計画承認で 3 案（A／B／C）から (A) を選び、web での成立形 (A2) は意思決定者が同日に確定した。実装はこの決定に従う。

- **採用案**: **新しい保存器・新しい記録種別は作らない。** やり取り自体は会話履歴（`sessions` / `messages`）に既に残っており、メンタリング記録（`decisions.kind='mentoring'`）は `session_id` を持つ（S3 の確証 1）。S3 は**その記録からそのセッションの会話へ辿れる導線**を足すことに絞る。web での成立形は **(A2) 読み取り専用の会話面の新設**——記録カードの導線から `GET /api/sessions/:id/messages` を取得し、送信欄を持たない面に表示する。**サーバ変更なし**
- **理由**: S3 の確証 3 のとおり web は当日のセッションしか表示できず、「あとから振り返る」の主用途（翌日以降）には過去セッションを読む手段が要る。(A2) は既存 API の読み取りだけで成立し、活性セッション・送信経路・`mentoringTarget`（S1b）に触れない（確証 (T)・(U)）ため、受入基準を「導線の可否」「取得した会話がそのセッションのものであること」「404 と失敗の表示」「送信経路と相談中の状態に差分が無いこと」と機械的に書ける
- **代替案**:
  - **(B) ボスがやり取りの要約を記録するツール・記録種別の新設** — 却下（LLM の要約品質に依存し、会話履歴との二重記録になる）
  - **(C) S3 を置かない** — 却下（Issue #438 の「綿密に相談しても振り返りづらい」に対し、結論 1 件しか読み返せない状態が残る）
  - **(A1) 既存タイムラインへの pin**（記録のセッションを `selectTimelineSessions` の `pinnedSessionIds` に足して当日分と一緒に表示） — 却下（確証 (T)。活性セッション宛の送信欄と過去会話の表示が同居し、pin の解除・リロード時の復元など S1b と同種の状態管理を再び持ち込む）
  - **(A3) 当日セッションに限定**（記録のセッションが当日タイムラインに含まれるときだけチャットへ切り替えて境界へスクロール） — 却下（翌日以降を満たさない）
- **影響範囲**: `DecisionLog.tsx`（導線）・読み取り専用の会話面（新規）・`AppLayout.tsx`（面の state を `DecisionLog` の外に置く場合のみ）。`server/` は触らない

### 22. 導線の対象・粒度・表示内容（S3・Issue #438 判断6）

> 意思決定者が 2026-09-22 に決定した方針であり、実装はこの決定に従う。**ただし 2 点目（`task_id` を持たない記録の扱い）だけは仕様執筆時に置いた仮定**であり、意思決定者の確認を仕様 PR のレビューで受ける（下記）。

- **導線の対象は `kind='mentoring'` の記録カードに限る**。`kind='decision'` のカードにも `session_id` はあるが、S3 は「メンタリングの経過」を辿る仕組みであり、決定カードへ広げるかは後続候補として「やらないこと」に置く（広げない）
- **`task_id` を持たないメンタリング記録も対象に含める（明示的な仮定・2026-09-22）**。`kind='mentoring'` の記録は `task_id` が `null` になりうる（朝会の必須メンタリングと全日単位の随時メンタリング〔#411 / #491〕の結論。`mentoring-tool.ts:74` の `explicitTaskId ?? mentoringTaskId ?? null`）。これらは決定ログの「タスクに紐づかない決定」セクションに並ぶが、やり取りが `messages` に残っている点はタスク紐づきの記録と変わらず、`kind` で限定するという上の規則からも外れない。出自の表示は**決定ログでその記録が属するセクションの見出しと同じ文言**（タスク紐づきならタスクのタイトル、無しなら `UNASSIGNED_SECTION_TITLE`）を使い、新しい文言を作らない。**この点は意思決定者の残差（導線の対象＝`kind='mentoring'`、出自＝対象タスクのタイトルと日時）が触れていなかったケースを、仕様執筆時に「`kind` で限定する」という規則の側へ倒して置いた仮定である**（意思決定者の決定そのものではない）。受入基準の「`task_id` を持たない…」2 項目はこの仮定から生じており、意思決定者が「`task_id` を持つ記録に限る」と改める場合は、その 2 項目を否定側（導線を出さない）へ書き換える。可逆で、実装前に仕様 PR のレビューで確認できるため、本仕様はこの仮定のまま起票へ進める
- **`task_id` はあるが `task_title` が `null` の分岐（タスク行が消えた場合）は受入基準で固定しない**。確証 (L) のとおりタスクは削除できず到達不能であり、`groupDecisionsByTask` と同じ `#<task_id>` の防御的フォールバックを踏襲するだけとする
- **粒度はセッション単位**。記録の `created_at` による発言位置への位置付け・スクロールはしない。理由: S3 の確証 2 のとおり記録は発言単位の紐づけを持たず、位置付けは近似にしかならない。近似を約束すると、やりなおし（S3 の確証 6）で発言が消えた場合に約束が破れる
- **読み取り専用の面は「どの記録から開いたか」（対象タスクのタイトルと記録の日時）を表示する**。理由: 確証 (V)。同じセッションを複数の記録が指すとき、会話だけでは区別できない
- **やりなおしで消えたやり取りは、残っている会話をそのまま表示する**（欠落の検知・表示はしない）。理由: 欠落を検知するには「その記録のターンにどの発言があったか」を保持する必要があり、新しい保存器を作らない決定 21 と衝突する
- **面を開いている間も活性セッションの状態（`mentoringTarget`・相談中の保持・送信中）を変えない**。理由: S1b の状態を壊さない。面は `useChat` の状態を読みも書きもしない（確証 (U)）
- **代替案**: 決定カードにも導線を出す — 却下（上記）。記録の `created_at` に最も近い発言へスクロールする — 却下（近似の約束）
- **影響範囲**: `DecisionLog.tsx`（`kind` による出し分け）・読み取り専用の会話面（出自の表示）
...
> **判断7（タスク起点のメンタリングは朝会の必須メンタリングゲートを満たしてよいか）は 2026-09-22 に決定済み**であり、ここには残っていない。決定 20 を参照すること（採用は (1) 満たさない＝現状維持。判断材料と却下した選択肢は決定 20 に移した）。
>
> **S3 の論点（S3 の形・導線の対象・粒度・表示内容）は 2026-09-22 に決定済み**であり、決定 21・22 を参照すること。
...
>
> **S2 の論点（スライスの切り方・振り返り導線の起点・セクション id と移動の手段・遡る範囲・取得経路・切り詰めの方針・積む条件）は 2026-09-20 に決定済み**であり、ここには残っていない。決定 13〜19 を参照すること。**未決のまま残っているのは論点A だけ**である（S2b は #545 で出荷済みだが、配分は本仕様で固定していない）。
...
Issue #438 の判断 6「綿密に相談した記録の粒度——やり取りの経過まで残す必要があるか」は、**S3 の導線（メンタリング記録 → そのセッションの会話を読む読み取り専用の面）で充足とみなす**（決定 21）。やり取りは `messages` に既に残っているため、要約を別に記録することはしない。
...
| S2b | 対象タスクに紐づく過去の決定・メンタリング記録を、`listRecentDecisions` とは別経路の新規クエリで引き、メンタリングのターンのシステムプロンプトへ積む（新しい順 5 件・`content` ＋ `rationale`・種別ラベル付き・0 件なら積まない・合計 2,000 文字を上限に新しい側から詰めて省略通知を出す）。**server のみ・web 非変更・マイグレーション無し** | 3（＋テスト 3） | **出荷済み**（#545 / PR #549・`96d2e9b`） |
| S2a | 振り返り導線（タスクカード → 決定ログの当該タスクのセクションへ、状態 prop ＋プログラム的スクロールで移動。`DecisionLog` にセクション id を付与）。**web のみ・サーバ非変更** | 4（＋テスト 4） | **出荷済み**（#557 / PR #559・`dddba97`） |
| S3 | メンタリング記録（決定ログの `kind='mentoring'` カード）から、その記録が生まれたセッションの会話を**読み取り専用の面**で読み返せる導線（決定 21・22）。会話の取得は既存の `GET /api/sessions/:id/messages`。面は開いた記録の対象タスクと日時を示し、送信欄を持たず、活性セッション・「相談中」に触れない。**web のみ・サーバ非変更・マイグレーション無し**。朝会ゲートとの関係は決定 20（現状維持）で閉じた | 4-5（＋テスト 4-5）＝ M 相当 | **実装対象**（S2a・S2b 出荷済み・判断7 決定済み） |

実装対象: S3（S1・S1a・S1b・S2b・S2a は出荷済み）

> S1 の出荷条件「結論はサーバ側補完で確実にそのタスクへ紐づく」が成立するのは**起動ターン内に限られる**（#476）。これは S1 の受入基準の未達ではなく約束文と実装範囲の差であり、S1b で埋めた（「### Issue #476 の確証」節）。
>
> **本改訂の実装対象は S3** であり、決定 20〜22 に従う。規模の見立て（web 4-5 ファイル＋テスト）は目安であり、要件チケットが計画承認を通る時点で改めて見積もる。**#477 / #491 / #503 / #561 は本ファイルの範囲外**であり、それぞれ別の仕様ファイル・Issue で扱われている。

## やらないこと

- **朝会ゲートの判定ロジックの変更**（理由: 判断7 は決定 20 で (1) 現状維持に確定した。`mentoring-gate.ts` は 1 行も変えない）
...
- **メンタリングの経過（やり取り）の要約を記録するツール・記録種別の新設（決定 21 の代替案 (B)）**（理由: LLM の要約品質に依存し、会話履歴との二重記録になる。やり取りは `messages` に既に残っており、S3 はそこへ辿る導線だけを足す）
- **メンタリング専用の対話面の新設**（理由: 決定 2。`sessions` / `messages` の扱いが増える。**S3 の読み取り専用の面は対話面ではない**——送信欄を持たず、活性セッションに触れない）
- **過去セッションの会話を既存のチャットタイムラインへ pin して表示すること（決定 21 の代替案 (A1)）**（理由: 確証 (T)。活性セッション宛の送信欄と同居し、S1b と同種の状態管理を再び持ち込む）
- **`kind='decision'` の記録カードへの会話導線**（理由: 決定 22。S3 はメンタリングの経過を辿る仕組みであり、決定カードへ広げるかは後続候補として別に判断する）
- **記録の `created_at` による発言位置への位置付け・スクロール**（理由: 決定 22。記録は発言単位の紐づけを持たず〔S3 の確証 2〕、近似の約束はやりなおしで破れる）
- **やりなおしで消えたやり取りの欠落検知・表示**（理由: 決定 22。検知には新しい保存器が要り、決定 21 と衝突する）
- **記録を起点にしない過去会話の閲覧手段（セッション一覧・日付ナビゲーション・検索）**（理由: S3 は「メンタリング記録から辿る」導線に絞る。一般的な履歴閲覧は必要になったら別 Issue で扱う）
- **タスクごとのメンタリングを「いつ促すか」（着手時の促し・タスクカードの印付け）**（理由: 別 Issue #561 の範囲。決定 20 のとおり本仕様では参照のみ）
- **サーバ側の変更・DB マイグレーション（S3）**（理由: 決定 21。既存 API の読み取りだけで成立する）
...
> **S2b の範囲（出荷済み・#545・`96d2e9b`）。S3 では満たし直さなくてよいが、壊してもいけない。** 決定 16・決定 17・決定 18・決定 19 に対応する。
>
...
- [x] 新規クエリは、指定した `task_id` を持つ `decisions` 行だけを返す（他タスクの行・`task_id` が `null` の行を含まない）
- [x] 新規クエリは `kind = 'decision'` の行と `kind = 'mentoring'` の行の両方を返す
- [x] 新規クエリは新しい順（`created_at` 降順、同時刻は `id` 降順）で返す
- [x] 新規クエリは指定された件数上限を超える行を返さない
- [x] 件数上限で打ち切られるとき、返るのは新しい側の行である（古い側が落ちる）
- [x] 指定した `task_id` に紐づく行が 1 件も無いとき、新規クエリは空配列を返す

システムプロンプトへの投入（`persona-prompt.ts`・純粋関数）:

- [x] `mentoring` が真・`mentoringTaskId` が `tasks` に存在・記録が 1 件以上、の 3 つが揃うとき、システムプロンプトに対象タスクの過去記録のセクションが現れる
- [x] システムプロンプト上のセクションの順序が「対象タスク」→ `MENTORING_TARGET_TASK_INSTRUCTION` → 過去記録である（過去記録が前 2 つの間に挟まらない）
- [x] そのセクションの各行に、記録の `content` が含まれる
- [x] `rationale` を持つ記録の行には、その `rationale` が含まれる
- [x] `rationale` が `null` の記録の行には、根拠を表す句が現れない
- [x] 各行には種別ラベルが含まれる（`kind='decision'` は「決定」、`kind='mentoring'` は「メンタリング」。決定ログ画面の `KIND_LABEL` と同じ語彙）
- [x] 各行には記録の日時が含まれる（既存の「直近の決定」と同じ日時の書式で表示される）
- [x] 記録が 0 件のとき、このセクションは現れない（見出しだけのセクションを積まない）
- [x] `mentoring` が偽のターンでは、記録が渡されてもこのセクションは現れない
- [x] `mentoringTaskId` が `tasks` に存在しない id のとき、このセクションは現れない
- [x] このセクションの追加によって、既存の「直近の決定」セクションの内容は変わらない
- [x] このセクションの追加によって、既存の「対象タスク」セクションの内容は変わらない

長さの制御（決定 18）:

- [x] 記録の `content` ＋ `rationale` の合計が文字数上限を超えるとき、新しい側の記録がセクションに残り、古い側の記録が落ちる
- [x] 文字数上限によって省略・切り詰めが起きたとき、その旨を示す通知がセクション内に現れる
- [x] 省略・切り詰めが起きていないとき、その通知は現れない
- [x] **最新の 1 件が単独で**文字数上限を超えるとき、プロンプトの生成は失敗せず、その 1 件が上限の長さへ切り詰められて現れる（記録が 1 件しか無い場合も、複数件あって最新の 1 件だけが単独で超える場合も、同じように切り詰めて残す）
- [x] 件数上限（5 件）と文字数上限の両方が効く（件数に収まっていても、文字数上限を超える分は落ちる）

チャットルートでの結線（`chat-messages-route.ts`）:

- [x] `mentoring: true` ＋ 有効な `mentoringTaskId` を伴うリクエストでは、対象タスクに紐づく記録が最大 5 件プロンプトへ渡される
- [x] `mentoringTaskId` を伴わないメンタリングのターンでは、過去記録のセクションがプロンプトに現れない
- [x] メンタリングでない通常のターンでは、過去記録のセクションがプロンプトに現れない

既存契約の保全（S2b）:

- [x] `listRecentDecisions` は `kind = 'decision'` のみを返す（`kind='mentoring'` を SQL レベルで除外する #408 AC-42 の契約が変わらない）
- [x] 日報の抽出（`collect-daily-report-data.ts`）は `kind = 'mentoring'` の行を含まない（#408 AC-43）
- [x] 作業ログの抽出（`collect-work-log-data.ts`）は `kind = 'mentoring'` の行を含まない（#408 AC-44）
- [x] `mentoring-gate.ts` の `isMentoringComplete` の判定（`mentoringRecordCount > 0 && userMessageCount > 0`）は変わらない
- [x] `RECORD_MENTORING_TOOL` の `input_schema.required` は `["content"]` のままである
- [x] `task_id` の補完（決定 5）の挙動は変わらない
- [x] web 側のコードは 1 行も変更されていない（`web/` に差分が無い）
- [x] DB マイグレーションは追加されていない（`migrate.ts` の最大 version が 8 のまま）

### 振り返り導線（S2a・Issue #438）

> **S2a の範囲（出荷済み・#557・`dddba97`）。S3 では満たし直さなくてよいが、壊してもいけない。** 決定 14・決定 15 に対応する。
>
> `scrollIntoView` は jsdom に無い（確証 (R)）。**呼び出しの観測は `ChatView.test.tsx:1471-1495` と同じくプロトタイプ側を差し替えて行う**こと。

タスクカードの導線（決定 14）:

- [x] タスクカードの表示モードに、そのタスクの記録を決定ログで見る導線が表示される
- [x] その導線は会（朝会・夕会）の最中にも表示される（`adhoc` 限定にしない）
- [x] その導線は、記録が 1 件も無いタスクのカードにも表示される
- [x] その導線を押すと、表示中のビューが決定ログへ切り替わる
- [x] タスクカードの既存の操作項目（ステータス select・編集ボタン・「メンタリングする」ボタン）は変わらない
- [x] タスク画面には決定・メンタリング記録の一覧を表示しない（導線だけを置く）

決定ログ側（決定 15）:

- [x] 決定ログのタスク別セクションに、そのタスクの id から導かれる `id` 属性が付く
- [x] `task_id` を持たない記録のセクションにも `id` 属性が付く
- [x] 導線から遷移したとき、対象タスクのセクションの要素に対して `scrollIntoView` が呼ばれる
- [x] 記録が 1 件も無いタスクの導線を押したとき、決定ログは開くがスクロールは行われない
- [x] ナビゲーションから決定ログを開いたとき（導線を経由しないとき）は `scrollIntoView` が呼ばれない
- [x] カードの導線から決定ログを開いてスクロールしたあと、別のビューへ移動してからナビゲーションで決定ログを開き直すと、`scrollIntoView` が呼ばれない（対象が一度の遷移で消費される）
- [x] 記録が 1 件も無いタスクの導線から開いたあと、ナビゲーションで決定ログを開き直しても `scrollIntoView` が呼ばれない（スクロールしなかった場合も対象が消費される）
- [x] `scrollIntoView` を持たない実行環境でも例外を投げない
- [x] 決定ログの既存の表示（セクションの見出し・セクションの並び順・記録の並び順・記録の描画内容）は変わらない
- [x] サーバ側のコードは 1 行も変更されていない（`server/` に差分が無い）

### メンタリング記録から当該会話へ辿る導線（S3・Issue #438）

> **実装対象スライス S3 の範囲。** 決定 21・決定 22 に対応する。
>
> 会話の取得は既存の `fetchSessionMessages` を通るため、テストは `fetch` をモックして `GET /api/sessions/:id/messages` の応答を差し替える（既存の `chat-api.test.ts` / `use-chat.test.ts` の作法）。サーバの応答形は `docs/features/session-not-found-response-shape.md` の契約（404・`code: "session_not_found"`）を前提にしてよい。**基準の「表示される」は、描画された要素をテストで観測できること**を意味する（文言そのものは固定しない）。

記録カードの導線（決定 22）:

- [ ] 決定ログの `kind='mentoring'` の記録カードに、その記録のセッションの会話を読む導線が表示される
- [ ] `task_id` を持たない `kind='mentoring'` の記録カード（「タスクに紐づかない決定」セクション）にも、その導線が表示される
- [ ] `kind='decision'` の記録カードには、その導線が表示されない
- [ ] その導線は会（朝会・夕会）の最中にも表示される（`adhoc` 限定にしない）
- [ ] 記録カードの既存の描画内容（種別ラベル・日時・`content`・`rationale`）は変わらない
- [ ] 決定ログの既存の表示（セクションの見出し・セクションの並び順・記録の並び順・セクションの `id` 属性）は変わらない

読み取り専用の会話面（決定 21）:

- [ ] 導線を押すと、その記録の `session_id` を対象に `GET /api/sessions/:id/messages` が呼ばれる（他のセッション id では呼ばれない）
- [ ] 取得した会話が古い順に表示される
- [ ] 表示される各発言は、利用者の発言とボスの発言が区別できる
- [ ] 面には入力欄と送信ボタンが無い（発言を送る手段を持たない）
- [ ] `task_id` を持つ記録から開いたとき、面には対象タスクのタイトル（`task_title`）が表示される
- [ ] `task_id` を持たない記録から開いたとき、面には決定ログの同じセクションの見出し（`UNASSIGNED_SECTION_TITLE`）が表示される
- [ ] 面には、開いた記録の日時が表示される
- [ ] 同じ `session_id` を持つ 2 件のメンタリング記録（別タスク）のそれぞれの導線から、同じセッションの会話が開ける
- [ ] 取得が完了するまでは、取得中であることを示す表示が出る
- [ ] 応答が 404（`code: "session_not_found"`）のとき、セッションが見つからない旨の表示が出る
- [ ] 404 以外の失敗（ネットワークエラー・5xx）のとき、404 とは区別できる取得失敗の表示が出る
- [ ] 面を閉じると、決定ログの表示に戻る

活性セッション・送信経路の保全（決定 22）:

- [ ] 面を開いている間、`useChat` の `mentoringTarget` は変わらない（相談中なら相談中のまま、`null` なら `null` のまま）
- [ ] 面を開いても、`POST /api/sessions/:id/messages` は呼ばれない
- [ ] 面を開いても、`POST /api/sessions`（セッションの作成）は呼ばれない
- [ ] 面を開いても、`POST /api/sessions/:id/end` は呼ばれない
- [ ] `web/src/use-chat.ts` は変更されていない（差分が無い）
- [ ] `web/src/ChatView.tsx` は変更されていない（差分が無い）
- [ ] サーバ側のコードは 1 行も変更されていない（`server/` に差分が無い）
- [ ] DB マイグレーションは追加されていない（`migrate.ts` の最大 version が 8 のまま）

### 検証方法・品質ゲート

> S1・S1a・S1b・S2b・S2a では満たし済み。**S3 でも同じ 3 項目を満たすこと**（スライスごとに再度確認する）。
