...
- **(O) プロンプトの実測長は 2,220 文字で、既存の 1 ブロックの上限（4,000 文字）より小さい。** `server/dist` の `buildPersonaPrompt` を実際に呼んで計測した結果（タスク 7 件・直近の決定 5 件・当日随時チャット無し・`mentoring: true` ＋対象タスクあり）が 2,220 文字、同条件で `mentoring: false` が 1,565 文字。一方「当日の随時チャット」ブロックは単独で `MAX_TODAYS_ADHOC_MESSAGES_TOTAL_LENGTH = 4_000`（`persona-prompt.ts:475`）まで許容されている。**S2b が足す 5 件分（`content` ＋ `rationale`）はこの既存の許容幅に対して十分小さい**——決定 18 が文字数上限を設けない根拠。
...
buildPersonaPrompt(persona, { ..., mentoring, mentoringTaskId, <過去記録のフィールド> })
    ...
    context.tasks に在る かつ 記録が 1 件以上のときだけセクションを描画し、
    「対象タスク」セクションより後に置く（決定 19）
```
...
AppLayout         ハンドラは activeView を決定ログへ切り替え、同時に
   ...
   ↓ props
DecisionLog       対象タスクの id を受け取り、取得完了後にその
                  セクションの要素へプログラム的にスクロールする（決定 15）
                  セクションには id 属性を付ける
```
...
- **代替案**: (a) 件数＋期間（例: 直近 30 日） — 却下（上記）。(b) 文字数上限で新しい側から詰める — 却下（決定 18）
...
- **理由（文字数上限を設けない）**: 確証 (O) の実測のとおり、メンタリングターンのプロンプト全体が 2,220 文字であるのに対し、「当日の随時チャット」ブロックは単独で 4,000 文字（`MAX_TODAYS_ADHOC_MESSAGES_TOTAL_LENGTH`）まで許容されている。5 件分の記録はこの既存の許容幅に対して十分小さく、上限機構を先に作るのは YAGNI である。**必要と分かった時点で足せる**——切り詰めの作法は `selectTodaysAdhocMessages`（新しい側から詰めて超えたら打ち切り、省略通知を添える）が先例として使える
...
- **代替案**: (a) `content` のみ — 却下（上記。深掘りの材料が落ちる）。(b) `rationale` まで積んだうえで文字数上限を設ける — 却下（現時点では不要。可逆に足せる）
- **影響範囲**: `persona-prompt.ts`（セクションの整形）

### 19. 過去記録を積む条件（S2b）

> 意思決定者が 2026-09-20 に決定した方針であり、実装はこの決定に従う。

- **採用案**: **S1 の「対象タスク」セクションと同じ AND 条件**——`mentoring` が真、かつ `mentoringTaskId` が `context.tasks` に存在するときだけ積む。セクションの位置は「対象タスク」セクションより**後**
- **理由**: 対象タスクを解決できないターンに過去記録だけを積んでも、ボスはそれが何のタスクの記録か分からない。`mentoring` が偽のターンには `mentoringTaskId` が後段へ渡らない設計（`chat-messages-route.ts:310`）なので、この AND 条件は既存の結線とも整合する。「対象タスク」の後に置くのは、対象タスクの提示（何の話か）を読んでから履歴（これまで何があったか）を読む順序が自然であり、S1 の `MENTORING_TARGET_TASK_INSTRUCTION` が「上の『対象タスク』セクション」と位置を前提にした文言を持つため、その間に別セクションを挟まないためである
...
| S2b | 対象タスクに紐づく過去の決定・メンタリング記録を、`listRecentDecisions` とは別経路の新規クエリで引き、メンタリングのターンのシステムプロンプトへ積む（新しい順 5 件・`content` ＋ `rationale`・種別ラベル付き・0 件なら積まない）。**server のみ・web 非変更・マイグレーション無し** | 3（＋テスト 3） | **実装対象**（決定 13） |
...
- **過去記録セクションの文字数上限・切り詰め（S2b）**（理由: 決定 18。確証 (O) の実測に照らして現時点では不要であり、必要になったら `selectTodaysAdhocMessages` の作法で可逆に足せる）
