...
> **注記（#509）**: 直近 7 日の窓を残したまま終端 2 列を既定で畳む変更は `taskboard-column-width-terminal-collapse.md` で扱う（下の「折りたたむ」の却下は窓の代わりに畳む案に対するもので、窓で絞る本決定は維持される）。
...
> **注記（#509）**: 見出しに**窓内の件数**を出す改訂（`完了（直近 7 日・N 件）`）を `taskboard-column-width-terminal-collapse.md` の決定 2 で行った。範囲外の件数を出さないことは変わらない。
