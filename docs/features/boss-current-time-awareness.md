...
- **採用案**: ローカル表記とオフセット付き ISO を併記し、**日付・曜日・時分・オフセット付き ISO の 4 点**を含める。秒は含めない。
  出力例: `現在日時: 2026-09-05（土）14:32（ISO: 2026-09-05T14:32+09:00）`
...
- **不正な値の扱い**: 整形の前に「暦として妥当な ISO 8601 日時」であることを検証し、満たさない値は**原文をそのまま出す**。`new Date()` は存在しない日付を黙ってロールオーバーさせ（`2026-02-30T12:00:00Z` → 3/2）、`"0"` のような非 ISO 文字列も受理するため、パース可否のガードだけでは**捏造された締切**がプロンプトへ出て締切超過の判定・催促の根拠が狂う。代替表示（「解析不能な日時」等）へ置き換えないのは、ボスがその文言をそのまま発話しうるうえ、原文のほうが上流のデータ異常を追跡できるため。
- **既知の限界（DST）**: 保存日時のローカル整形には**オフセットを併記していない**（併記するのは「今」だけ）。そのため DST を採用するタイムゾーンで運用した場合、**DST 遷移をまたぐ区間の経過時間・残り時間の計算が最大 1 時間ずれうる**（23/25 時間の日を 24 時間と解釈する、fall-back で重複する時刻を区別できない）。現状の運用環境は `Asia/Tokyo` で DST が無く、固定オフセット下ではローカル壁時計表記が一意に定まるため実害が無いこと、および全保存日時へオフセットを併記するとプロンプトが冗長になることから、現時点では対応しない。**DST のある地域で運用する場合はこの決定を再検討すること。**
...
- [ ] `decidedAt` が暦として妥当な ISO 8601 日時でない文字列（`Date` が解釈できない値・存在しない日付・非 ISO 表記）のとき、その値をそのまま表示する
- [ ] `reportedAt` が暦として妥当な ISO 8601 日時でない文字列（`Date` が解釈できない値・存在しない日付・非 ISO 表記）のとき、その値をそのまま表示する
- [ ] `due_at` が暦として妥当な ISO 8601 日時でない文字列（`Date` が解釈できない値・存在しない日付・非 ISO 表記）のとき、その値をそのまま表示する
