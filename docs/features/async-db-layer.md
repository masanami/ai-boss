...
> **草案（2026-09-26）**: クリティカル設計決定の各節は、親（flywheel エージェント）とオーナーの回答待ちの論点を含む（Q1: 決定 1・2／Q2: 決定 3／Q3: 決定 6／Q4: 決定 5／Q5: 決定 7）。「推奨」と書いた案は未確定である。決定 4 は ADR 0005 決定 4 の維持であり論点にしない。
...
- [ ] ADR 0005 の決定 1・5 と帰結の「複数端末からの同時書き込みは前提外」を、「クリティカル設計決定 6」の方向で書き換える（S2・Q3。文面は S2 の実装で具体が決まってから確定する）
...
- **推奨案**: 非同期化で割り込めるようになる「トランザクションの外で読み、その結果を根拠に中で書く」箇所は、**判定に使う読み出しもトランザクションの中へ入れる**。S1 の対象は T1〜T9 の周辺で実測した次の 5 箇所に限る（トランザクションを張っていない読み→書きの箇所は S1 の対象外。「やらないこと」参照）。
  ...
  | T4 会議時刻 | `buildMeetingDefaults`（既定の時刻） | 既定の時刻の変更と並行すると、既定と同じ値の上書きが削除されずに残る（表示は変わらない） |
  | T6 発言の書き直し | `findSessionById`（終了済みの検査）・`findMessageInSession` | 会の終了と並行すると、終了済みの会へ書き直しが入る |
...
### 6. ADR 0005 の改訂の方向（Q3・未確定。改訂の本体は S2）

- **推奨案**: ADR 0011 の帰結に従い、改訂の本体は S2（両版の実装がそろった時点）で行う。方向は次のとおり。
  - 決定 1: 「better-sqlite3 の同期 API・単一ライター」→「非同期の DB ポート越しに使い、ドライバは版ごと（開発者用: better-sqlite3／製品版: plugin-sql の fork）。**接続は 1 本**」
  ...
  - 帰結「複数プロセス・複数端末からの同時書き込みは前提外」→「端末内は単一ライターのまま。端末間の変更は同期の層（ADR 0011 決定 18）が取り込む。衝突解決の方式は Q4 の結論に従って書く」
- **代替案**: S1 で決定 1・5 を先に改訂する — S1 の時点では製品版のドライバが無く、ADR 0011 の帰結（実装で具体が決まってから改訂）に反する。

### 7. S2（製品版の実装）と #579 S2（Tauri の器）の順序（Q5・未確定）

- **事実**: #579 の機能仕様は S2 の出荷条件を「S1 と #580 がマージされてから」としている。一方、本機能の S2（plugin-sql の fork と製品版の実装）は、動かして確かめる Tauri のアプリが要る。どちらも相手を先に要する。
- **推奨案**: #579 S2 の出荷条件を「#580 **S1** がマージされてから」に読み替え、本機能の S2 は #579 S2 の器の上で行う（製品版の実装と契約テストを器の中で走らせる）。
- **代替案**: 本機能の S2 で検証用の最小の Tauri アプリを別に作る — #579 S2 の器と二重になる。
...
| S2 | 製品版の plugin-sql 実装（Rust 側の fork で接続 1 本）と、S1 の契約テストを Tauri 上で通す仕組み。ADR 0005 の改訂 | 10-20 | S1 がマージされてから（#579 S2 との順序は決定 7・Q5） |
...
- [ ] AC-2b: `transaction` の中で投げられた例外は、そのまま呼び出し元へ伝わる
...
- [ ] AC-19: T4（会議時刻）と T6（発言の書き直し）で、判定に使う読み出しがトランザクションの中で行われている（要人間判定: 並行の順序をテストで再現しにくいため、コードレビューで確認する）
