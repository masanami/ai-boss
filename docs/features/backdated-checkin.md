...
- **2 回の POST が原子的でないことへの対処**（この残差は web で潰す）:
  ...
  - **「`break_start` は記録済み・`break_end` 未記録」の状態を活動一覧の導出とは独立に保持する**（Codex 指摘・PR #354）: 後追いする休憩より後に閉じた休憩が既にあると「戻りました」表示に切り替わらないため、この状態の間は「休憩」ボタンが `break_end` だけを再送し、`break_start` を二重記録しない。展開欄にその旨を表示する。「戻りました」での再送成功、または記録する時刻の変更で解除する
