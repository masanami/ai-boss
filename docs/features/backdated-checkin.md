...
> **状態: 要件確定中（#243）。実装を駆動する作業文書（非権威）。**
>
> **正はコードとテスト**であり、本ファイルは権威を持たない（`CLAUDE.md`「開発方針」）。実装と食い違う場合はコードが正。
>
> 論点 0〜5 の決定はオーナーが計画承認ゲートで承認済みのものを転記した（2026-09-05・main = `04aab29` で裏取り）。**「休憩の後追い記録の形（判断 6）」のみ親セッションの回答待ち**（該当箇所に【未確定】と付す）。
...
`activity_events` は検知エンジン・エスカレーション・日報・活動ログが**時刻で範囲を切って読む**ため、過去時刻のイベントを差し込むと各消費者に影響する。本仕様は Issue #243 の論点 1〜5 それぞれについて期待する挙動を確定し、どの層に規律が及び、どの層には手を入れないかを明記する。
...
- [ ] チェックインパネルから時刻を指定して 着手 / 一時停止 / 休憩 / 戻り / チェックイン を記録できる
- [ ] 過去時刻の `task_start` / `task_pause` は、それが当該タスクの最新の着手・一時停止イベントより新しいときだけ `tasks.status` を遷移させる
- [ ] 未来時刻・当日ローカル暦日より前の時刻・直前の `break_start` より前の `break_end` は 400 で拒否される
- [ ] 後追い記録によって送信済み通知・エスカレーション履歴・生成済み日報は変更されない
...
- 既存コードとの関係:
  ...
  - 格納時は既存の `created_at` と同じ `toISOString()` 形式（UTC・ミリ秒付き）に正規化する。`listEventsSince` 等は `created_at` を**文字列比較**で範囲切りしているため、オフセット付き文字列をそのまま格納すると範囲判定が壊れる
...
- `occurred_at`: 省略可。指定する場合は `isValidIsoDateTime` を満たす ISO 8601 **日時**文字列（`YYYY-MM-DD` の日付のみは 400）。`null` は省略と同義
...
- 展開すると `記録する時刻` の時刻入力（当日内なので**時刻のみ**）が現れる。値が入っている間は、既存の 着手 / 一時停止 / 休憩 / 戻りました / チェックイン の各ボタンが `occurred_at`（当日ローカル暦日のその時刻）を付けて送信する
- 展開欄は休憩中（「戻りました」だけが出る状態）でも使えるようにする（休憩からの戻りも後追いしたいため）
...
- **影響範囲（及ぶ層）**: **`detection/escalation.ts` は無変更**。回帰テストで「`sent_at` より前の後追いイベントはリセットしない／後ならリセットする」の両側を、API 経由で記録したイベントを `resolveEscalation` に渡す形で固定する
...
### 判断 6: 休憩の後追い記録の形 【未確定・親の回答待ち】

上の調査結果のとおり、`break_start` だけを後追いすると「既に再開しているのに休憩超過が飛ぶ」「後続の休憩があると閉じ忘れに気付けず日報が過大計上される」が起きる。承認済み設計はこの点を扱っていないため、次の選択肢を親へ提示している。

- **案 A: 現状の解釈を受け入れる** — `break_start` の後追いは単独イベント。パネルが「戻りました」に切り替わるのを合図に、ユーザーが戻り時刻を指定して `break_end` を後追いする。両者の間に tick が入れば `break_overrun` が 1 回飛ぶことを仕様として明記する。実装は最小。後続の休憩が既にある場合は合図が出ない
- **案 B（推奨）: 時刻指定モードの「休憩」は開始時刻と任意の戻り時刻を一度に記録する** — 展開欄に `戻り時刻（任意）` を足し、入力があれば web が `break_start`（開始時刻）→ `break_end`（戻り時刻）の順に 2 回 `POST /api/checkins` する。空なら `break_start` のみ（「まだ休憩中」の宣言＝案 A と同じ挙動で、催促が出るのは自然）。サーバは判断 5 の順序検証だけで足り、エンジン・日報は無変更。閉じ忘れの窓が実質消える
- **案 C: `task_start` が休憩を閉じるようエンジンを変える** — 不採用。`getActiveBreak` / `pairBreaks` / `deriveIsOnBreak` の 3 箇所の解釈（ADR 0007 帰結）を変える横断変更で、本課題の「検知エンジン無変更」の線引きを超える

本文の画面設計・受入基準は**案 B を前提に書いてある**。案 A に決まった場合は `戻り時刻` に関する記述と AC-19〜AC-21 を削る。
...
2. **web**: 展開式の時刻指定欄・`occurred_at` 送信・事前チェック・成功時の一言（判断 3）・（案 B なら）戻り時刻の同時記録
...
- [ ] AC-18: チェックインパネルの「時刻を指定して記録」を展開して時刻を入力すると、着手 / 一時停止 / 休憩 / 戻りました の送信に当日ローカル暦日のその時刻が `occurred_at` として付く
- [ ] AC-19: 【案 B】時刻指定欄に戻り時刻を入力して休憩を記録すると、`break_start`（開始時刻）の後に `break_end`（戻り時刻）が送信される
- [ ] AC-20: 【案 B】戻り時刻が空のまま休憩を記録すると、`break_start` のみが送信される
- [ ] AC-21: 【案 B】戻り時刻が開始時刻より前のとき、休憩ボタンは無効になる
- [ ] AC-22: 時刻指定欄を展開していない（または時刻が空の）とき、送信に `occurred_at` は付かない
- [ ] AC-23: 時刻指定欄に未来の時刻を入力すると、各送信ボタンが無効になり理由が表示される
- [ ] AC-24: 時刻を指定した記録の成功後、フィードバックに日報の再生成が必要な旨が含まれる
- [ ] AC-25: サーバが 400 を返したとき、そのメッセージが `role="alert"` の要素に表示される
- [ ] AC-26: 時刻指定欄を折りたたんでいるとき、既存のボタン・入力の構成は現行と同じである
