...
- **2 回の POST が原子的でないことへの対処**（この残差は web で潰す）:
  ...
  - **応答が失われた `break_end` の突き合わせ**（Codex 指摘・PR #356/#357）: サーバが `break_end` を記録したのに応答が失われた／解釈できなかった場合、直列送信は 2 件目失敗として返るが、送信後に再取得した活動一覧に同じ時刻の `break_end` があれば記録完了として扱い、保留にしない（保留にすると再送が同時刻重複の 400 で永久に弾かれる）。`submitCheckins` は再取得した一覧を返し、呼び出し側がこの突き合わせを行う
