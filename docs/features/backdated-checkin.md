...
- **2 回の POST が原子的でないことへの対処**（この残差は web で潰す）:
  ...
  - **POST の失敗と送信後の再取得失敗を区別する**（Codex 指摘・PR #354）: 2 件の送信は専用の直列送信（`submitCheckins`）で行い、`break_start` の 201 の後に活動一覧の再取得だけが失敗しても `break_end` は送る（再取得の失敗は活動一覧側の表示に委ねる）
  - **「`break_start` は記録済み・`break_end` 未記録」の状態を活動一覧の導出とは独立に保持する**（Codex 指摘・PR #354）: 後追いする休憩より後に閉じた休憩が既にあると「戻りました」表示に切り替わらないため、この状態の間は「休憩」ボタンが `break_end` だけを再送し、`break_start` を二重記録しない。展開欄にその旨を表示する。「戻りました」での再送成功、または記録する時刻の変更で解除する。**保留中の開始時刻・戻り時刻は展開欄の入力とは独立に保持する**（Codex 指摘・PR #356/#357: 展開欄を折りたたむと入力値が `null` になり、保留フラグだけが残ると「休憩」が `break_start` を二重記録し「戻りました」が現在時刻で閉じてしまう）。保留の分岐は「戻り時刻が空か」の判定より前に置き、折りたたんだままでも保留時の戻り時刻（展開欄に戻り時刻があればそちらを優先）で `break_end` だけを再送する。保留バナーは展開欄の外に出す
  - **応答が失われた時刻指定付き POST の突き合わせ規律**（Codex 指摘・PR #356/#357）: サーバが記録したのに応答が失われた／解釈できなかった `occurred_at` 付きの POST は、送信直後に再取得した活動一覧に**同じ `type` と `created_at`（= `occurred_at`）**のイベントがあれば送信済みとして扱う（保留にしない。保留にすると再送が同時刻重複の 400 で永久に弾かれる）。この規律は **`use-checkin-panel.ts` の 1 箇所（`postWithReconcile`）**で実装し、`submitCheckin` と `submitCheckins` の両方がそこを通る。したがって次のすべての経路に同じ規律が及ぶ（`occurred_at` の無い POST はサーバ時刻で記録されるため突き合わせできず、従来どおり失敗として扱う）:
    1. 通常経路・休憩 2 件送信の 1 件目 `break_start`（`submitCheckins`）→ 突き合わせて送信済みなら 2 件目へ進む
    2. 通常経路・休憩 2 件送信の 2 件目 `break_end`（`submitCheckins`）
    3. 保留経路・「休憩」からの `break_end` のみ再送（`submitCheckins`）
    4. 保留経路・「戻りました」からの `break_end` 再送（`submitCheckin`）
    5. 単発の時刻指定 POST（着手 / 一時停止 / 休憩開始のみ / 戻りました）（`submitCheckin`）
  - **存在しない現地時刻の拒否**（Codex 指摘・PR #357）: `HH:mm` から当日ローカル暦日の `Date` を組み立てたあと、その現地時・分が入力と往復一致することを検証し、一致しない（夏時間の春の切り替えで飛ぶ時刻。例: `America/New_York` の 2026-03-08 02:30）場合は入力エラーとして表示し送信を止める。`Date` コンストラクタが黙って 03:30 へ丸めた値を送らない
...
- [ ] AC-35: 戻り時刻付きの休憩で `break_start` の 201 の後に活動一覧の再取得だけが失敗しても、`break_end` は送信される
- [ ] AC-36: 戻り時刻付きの休憩で 2 回目の `break_end` が失敗し、かつ後続の閉じた休憩があって「戻りました」表示にならない場合でも、「休憩」を押し直すと `break_start` を再送せず `break_end` だけが送信される
- [ ] AC-37: `occurred_at` を指定した `break_end` は、同じ時刻に `break_end` が既にあるとき 400 を返す（重複記録しない）
