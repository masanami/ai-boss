...
- 依存関係: **S2 以降は #580（DB 層）の完了を待つ**（better-sqlite3 は WebView で動かないため）。**LLM を使う流れの Tauri アプリでの確認は #581 の完了を待つ**
...
- #580 が完了するまでは、DB に触れる経路をアプリ内で動かさない（S1 は DB を実行しない）。
- ADR 0002 改訂が「アプリ内のどの層が永続状態と DB への副作用を受け持つかは、実行の仕組みを作り替える移行 Issue で決め、そのときに本 ADR を改訂する」としている。本機能では「**WebView 内の TS コア（Hono アプリ）が受け持つ**」とし、ADR 0002 の改訂は Tauri アプリが DB を実行するスライス（S2）で行う。
...
- 順序: **S1 コアの Node 依存切り離し → S2 Tauri の器（#580 待ち）→ S3 スケジューラ・通知・常駐 → S4 証跡ファイルの保存**。
...
| S2 | Tauri 2（macOS）の器。製品版のエントリで `/api` をアプリ内の `app.fetch` へ振り向け、既存の画面が #580 の DB で動く。証跡の `<a href>` の Blob URL 化。WKWebView 上の SSE と生成停止の実機確認。ADR 0002 の改訂 | 10-20 | S1 と #580 がマージされてから |
