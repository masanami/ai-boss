...
| S2 | Tauri 2（macOS）の器。製品版のエントリで `/api` をアプリ内の `app.fetch` へ振り向け、既存の画面が #580 の DB で動く。証跡の `<a href>` の Blob URL 化。WKWebView 上の SSE と生成停止の実機確認。ADR 0002 の改訂 | 10-20 | S1 と #580 の S1（#597）がマージされてから（#580 の S2 は本機能の S2 の器の上で行う） |
