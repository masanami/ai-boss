...
> **状態: 実装完了（#179・`cd0ffe0` でマージ済み）。経緯の記録として保持する作業文書（非権威）。**
>
> **正はコードとテスト**であり、本ファイルは権威を持たない（`CLAUDE.md`「開発フェーズ」）。実装と食い違う場合はコードが正。
>
> 本文には退役した保証台帳の保証 ID（`G-NNN-N`）への参照が残っているが、**台帳は GDD 不採用の決定（2026-08-21）により退役済み**であり、これらは解決先の無い参照である。**判断当時の記録**としてそのまま残しており、書き換えていない。
>
> 現在の振る舞いの正本は**コードと `*.test.ts`**。恒常的な設計決定は `docs/adr/` にある。本ファイル自体は SDD 期の退役手順（ADR 昇格の要否判定 → ファイルごと削除）で片付ける。
