...
> 対象 Issue: #561
>
> 判断 1〜4（促しの強さ・トリガー・「未確認」の定義・頻度と朝会の印付けの形）は**プロダクトオーナーが 2026-09-22 に決定**した（決定 1〜4）。実装面の設計（決定 5〜8）は意思決定者（親エージェント）の設計方針を本仕様が実コードで確証して確定したもので、S1 の範囲（朝会の印付けを S2 へ分けること）は親の委任範囲で本仕様が判断した（決定 5）。
>
...
> 後続の実装はこの決定に従い、独自判断で逸脱しない。決定 1〜4 はプロダクトオーナー（2026-09-22）、決定 5 は本仕様（親の委任範囲）、決定 6〜8 は親の設計方針を実コードで確証したものである。
...
- **注記**: 判定（`fetchDecisions()`）は非同期であり、取得完了時点で促しを表示する。取得中にそのタスクの状態がさらに変わっても追わない（明示的な仮定）。**異なるタスクの遷移が連続したときは取得の完了順が遷移の発生順と逆転しうる**ため、**遷移を 1 件検知するごとに世代番号を 1 つ進める**（`fetchDecisions()` を呼ぶかどうかにかかわらず。`estimated_minutes` が `null` で取得を省いて同期的に「未確認」と判定するケースでも進める）。取得を伴う判定は呼び出し時の世代番号を記録し、取得完了時点でそれが最新の世代番号でなければ表示を更新しない（`web/src/use-tasks.ts` の `refresh()` が `generationRef`（29-43 行）で「呼び出しごとに進め、完了時に比較する」確立パターンに倣う）。これにより AC-16・AC-16b の「最後に発生した遷移のタスク 1 件だけ」が取得の完了順に依存せず成立する

## 機能全体の設計

### アーキテクチャ決定

- 変更は web のみ。`AppLayout` に (1) 前回 `tasks` との比較で遷移を抽出、(2) 未確認判定（`estimated_minutes` → 必要なら `fetchDecisions()`）、(3) 促しの対象タスクと促し済み集合の state、(4) 促しコンポーネントの描画、を足す
...
- [ ] AC-16b 先に遷移したタスク A（`estimated_minutes` 非 `null`・`GET /api/decisions` の取得待ち）の取得が、後に遷移したタスク B（`estimated_minutes` が `null`）の促しが表示された後に完了しても、表示される促しは B の 1 件のままで A に置き換わらない
...
8. 判定の取得が完了した時点で促しを表示し、取得中の状態変化は追わない（決定 8）
