...
設定画面の保存で出る `PUT /api/settings` の 400 のうち、利用者の入力値に対する 5 つの規則（勤務時間の前後関係・必須の文字列・時刻の形式・正の整数）の応答を変える。英語の生の文言をやめ、「日本語の `error`（どの項目をどう直せばよいか分かる文）＋機械可読な `code`」にする。**保存の可否（どの値を拒否するか）とステータス 400 は変えない。** エラーの伝え方だけを変える。web は変えない。
...
- [ ] 必須の文字列（`boss_name` / `model`）の空欄を拒否する 400（`:64`）が、キーごとに決定 3 の日本語の `error` と `code` を返す
- [ ] 時刻の形式（`work_start` / `work_end` / `morning_meeting_time` / `evening_meeting_time`）で拒否する 400（`:114`）が、キーごとに決定 3 の日本語の `error` と `code` を返す
- [ ] 正の整数（検知閾値 6 キー）で拒否する 400（`:123`）が、キーごとに決定 3 の日本語の `error` と `code` を返す
- [ ] 上記の拒否で、保存の可否とステータス 400 は変更前と同じである
...
- [ ] 時刻の形式の検証（`:114`）から `code` だけを取り除く変異を加えると、時刻 4 キー（`work_start` / `work_end` / `morning_meeting_time` / `evening_meeting_time`）それぞれの形式のテストが落ちる
- [ ] 時刻の形式の検証（`:114`）の `error` だけを変更前の英語に戻す変異を加えると、時刻 4 キーそれぞれの形式のテストが落ちる
- [ ] 正の整数の検証（`:123`）から `code` だけを取り除く変異を加えると、検知閾値 6 キーそれぞれの正の整数のテストが落ちる
- [ ] 正の整数の検証（`:123`）の `error` だけを変更前の英語に戻す変異を加えると、検知閾値 6 キーそれぞれの正の整数のテストが落ちる
