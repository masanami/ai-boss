...
**「完了が勝つ」はクライアント側にも同じく課す**（PR #287 のレビューで実装漏れが判明）。`sendChatMessage` は `done` を配信したあとも EOF まで読み続けるため、「応答は完成したが read が pending」という窓がある。そこで停止すると abort が pending な read を reject させるが、応答は完結しており `onDone` が既に完了エントリを積んでいるので、**中断エントリを重ねて積んではならない**（同じ応答が 2 通、しかも 2 通目が中断扱いで並ぶ）。

**停止可能になるのはメッセージ POST を投げられる時点から**（同レビューで判明）。セッション未作成の初回送信では `POST /api/sessions` の往復のあいだに停止されうるが、その時点で signal を abort 済みにすると続く `fetch` が即 reject して**ユーザーの発言がサーバへ届かない**（楽観追記のせいで画面には残るがリロードで消える ＝ 完了条件「ユーザーの発言は履歴に残る」に反する）。この窓での停止は honor しない — まだ何も生成されておらず、honor する唯一の方法がユーザーの発言を捨てることになるため。
