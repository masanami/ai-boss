...
> 対象 Issue: #510
>
> 方向（案 A）と論点 A1〜A6 はプロダクトオーナーが、設計論点 B1〜B6 は意思決定者（親エージェント）が、いずれも 2026-09-13 に決定した。本仕様の決定 4 の一部（勤務時間外の評価を暦日で区切る・休憩ゲートの例外・最優先タスクの繰り下げをしない）は、それらの回答から本仕様が導いた解釈であり、その旨を各所に明記する。
...
| C15 | 暦日の半開区間の境界を求める関数は server に 1 箇所だけある（`startOfLocalDayIso` / `startOfNextLocalDayIso`） | `server/src/activity/local-day.ts`・`docs/adr/0007-local-calendar-day-basis.md` 帰結 |
...
- [ ] 約束の催促は、勤務時間帯外でも約束の暦日のうちは評価される
- [ ] 約束の催促は、勤務時間帯内では約束の暦日を過ぎても評価され続ける（約束は失効しない）
- [ ] 約束の催促は、休憩申告中も評価される
...
- **暦日の境界は `server/src/activity/local-day.ts` の既存関数で求める**。日付の切り出しを新たに書かない（ADR 0007 帰結）
- **テストの固定時刻はローカル日時（`new Date(y, m, d, h, min)`）から導出し、TZ 非依存に組む**（ADR 0007 決定 5）。暦日の区切りに触るため `npm run test:tz` も通し、`main` のベースラインとの差分で判定する（同 決定 6）
...
- **採用案**:
  ...
  4. **エスカレーション**: 既存の `resolveEscalation` をそのまま通す。したがって「前回通知より後に活動シグナルが 1 件でもあれば L1 にリセットして即発火する」挙動（C5）も**継承する**
  ...
  6. **勤務時間帯外ゲート**: `commitment_missed` は勤務時間帯外でも評価する。ただし勤務時間帯外では、`now` が約束の時刻のローカル暦日 `[D 00:00, D+1 00:00)` に含まれる間だけ評価する。勤務時間帯内では暦日を問わず評価する
  7. **休憩ゲート**: `commitment_missed` は休憩申告中も評価する
  8. `silence`・`deadline_overdue`・`break_overrun`・朝会・夕会の各ルールは変えない
- **理由**:
  - 1〜4 は B4 の回答どおり。3 により、リスケした約束は新しい段階の列として L1 から始まる
  - 5 の「約束の前後を問わない」は B4 の回答（約束前は催促せず、約束後は新ルールに一本化）の直訳。「次点へ繰り下げない」は本仕様の解釈である。繰り下げると、約束したタスク A に取り組んでいる間に、次点のタスク B に対して「他のタスク（A）に活動がある」として回避の催促が出る
  - 6 の勤務時間帯外の評価は A4 (b)・B4 の回答どおり。**暦日で区切るのは本仕様の解釈**である。区切らないと、B3 の「失効させない」と合わさって、着手するか約束を変えるまで L3 が 10 分ごとに一晩中続く。B4 の回答が「朝会・夕会と同様に勤務時間ゲートの例外に置く」としており、朝会・夕会の定時催促は暦日ごとにリセットされる（C6）ため、同じく暦日で区切った
  - 7 の休憩ゲートの例外も**本仕様の解釈**である（B4 の回答は勤務時間ゲートにだけ触れていた）。勤務時間帯外は休憩延伸検知も止まっているため、休憩ゲートを当てると閉じ忘れた休憩 1 件で夜の約束の催促が無言で消える。朝会・夕会と同じく両ゲートの外に置いた
- **代替案**:
  - 未着手ルールの起点だけを約束時刻へ差し替える — 却下。最優先でないタスクの約束を見ない（A3 件数 (b) に反する）
  - 最優先タスクの選び方に約束を反映する — 却下。回避の判定対象まで巻き込んで変わり、約束を過ぎたタスクが複数あると 1 件しか見ない
  - 勤務時間帯外は暦日を問わず評価する — 却下（上記の一晩中の L3）
...
- **通知後の活動シグナルで L1 に即リセット・即再発火する既存のエスカレーション挙動の変更**（理由: 未着手・回避と共通の既存挙動。`commitment_missed` もこれを継承する〔決定 4 の 4〕が、挙動そのものは変えない）
...
- [ ] `evaluateRules` は、同じ `ruleKey` で L1 を 14:00 に送った後、活動シグナルが無ければ `now` 14:15 で `commitment_missed` を L2 で返し、14:14 では返さない（変異: エスカレーションを通さず常に L1 で返す — 入力 `now` 14:14 で L1 が返る）
- [ ] `evaluateRules` は、同じ `ruleKey` で L1 を 14:00 に送った後に活動シグナル（`chat_message` 14:05）があると、`now` 14:06 で `commitment_missed` を L1 で返す（既存の即リセットの継承。変異: `commitment_missed` だけ活動によるリセットを無効にする — 入力 `now` 14:06 で間隔未経過として返らなくなる）
- [ ] `evaluateRules` は、約束 20:00 のタスクに対し、勤務時間帯外の `now` 20:00 で `commitment_missed` を返す（変異: `commitment_missed` を勤務時間帯ゲートの内側で評価する — 入力 `now` 20:00 で返らなくなる）
- [ ] `evaluateRules` は、約束が 2026-09-14 20:00 のタスクに対し、勤務時間帯外では `now` 2026-09-14 23:59 で `commitment_missed` を返し、2026-09-15 00:00 では返さない（変異: 暦日の上限を `<` から `<=` にする、または暦日の区切りを外す — いずれも入力 `now` 2026-09-15 00:00 で返る）
- [ ] `evaluateRules` は、約束が 2026-09-14 20:00 のタスクに対し、勤務時間帯内の `now` 2026-09-15 09:00 で `commitment_missed` を返す（変異: 暦日の区切りを勤務時間帯内にも当てる — 入力 `now` 2026-09-15 09:00 で返らなくなる）
- [ ] `evaluateRules` は、休憩申告中（終了していない `break_start`）でも `commitment_missed` を返す（入力は勤務時間帯内「`break_start` 13:55・約束 14:00・`now` 14:00」と勤務時間帯外「`break_start` 19:55・約束 20:00・`now` 20:00」の 2 通り。変異: 休憩ゲートの内側で評価する — 勤務時間帯内の入力で返らなくなる。変異: 勤務時間帯外でだけ休憩ゲートを当てる — 勤務時間帯外の入力で返らなくなる）
