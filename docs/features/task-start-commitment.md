...
- [ ] タスクのステータスが `todo` 以外へ変わると、着手の約束は退役する（消え、更新記録に残る）。後で `todo` に戻しても約束は復活しない
...
### 決定 3-2: ステータスが `todo` 以外へ変わったら、同じ更新で約束を退役させる

- **採用案**:
  - `updateTask` で**ステータスが変わり、変更後のステータスが `todo` 以外（`in_progress`・`paused`・`done`・`dropped`）になる更新**では、約束を持っていれば同じ更新で `committed_start_at` を `NULL` にする（退役）。遷移元は問わない（`todo` からに限らない）
  - 退役したとき、決定 3 の `note` に「約束の退役（ステータス変更による）」と変更前の値を残す。同じ更新でエビデンス要否も変わった場合は、決定 3 のとおり 1 つの `note` に両方を含める
  - 同じ更新で `committed_start_at` に新しい値が送られても、ステータスが `todo` 以外へ変わる更新では退役を優先して `NULL` にする
  - エビデンス強制のゲートで拒否された更新（何も書き込まれない。C12）では退役しない
  - ステータスが変わらない更新（タイトルだけの変更など）と、変更後が `todo` になる更新では退役しない
  - `insertTask`（作成）でステータスが `todo` 以外のときは、`committed_start_at` が送られても `NULL` で保存する
...
- **ステータスが変わる経路（実コードで洗い出した）**: いずれも `server/src/tasks/tasks-repository.ts` の `updateTask` を通る。
  ...
  作成（`POST /api/tasks` と `create_task`）は `insertTask` を通る。**退役は経路ごとに書き分けず、`updateTask` と `insertTask` の 2 箇所（リポジトリ層）に置く**。経路側に置くと、新しい経路を足したときに退役が漏れるため
- **理由**: 約束は「未着手のタスクをいつ始めるか」であり、着手・一時停止・完了・取り下げのいずれかでステータスが `todo` を離れた時点で役目を終える。残すと、着手して約束を果たした後に `todo` へ戻したタスクへ、果たした約束について誤って催促する（Codex レビュー指摘）。遷移元を `todo` に限らないのは、`paused` のタスクに置いた約束が `in_progress` を経て `todo` に戻る場合にも同じ誤催促が起きるため（親の決定「`todo` から他へ移った時点」を、同じ欠陥を塞ぐ範囲へ広げた）
- **代替案**:
  - 退役させず、`commitment_missed` の判定で「約束の時刻より後に `task_start` があれば対象外」とする — 却下。活動履歴の読み方を検知側に足すことになり、`todo` に戻したタスクの約束が画面とボスの文脈に残り続ける
  - 経路（ルートハンドラ・ツール）ごとに退役を書く — 却下（上記の漏れ）
- **影響範囲**: `server/src/tasks/tasks-repository.ts`（`updateTask`・`insertTask`・`note` の組み立て）
...
1. 保存層: マイグレーション・`Task` 型（server）・検証と正規化・`updateTask` の `note`・ステータス変更による約束の退役（決定 1〜3-2）
...
- [ ] ボスの `update_task` ツールで約束を持つ `todo` のタスクのステータスを変えると、`committed_start_at` が `null` になる（変異: 退役を `PATCH /api/tasks/:id` のルートハンドラにだけ置く — 入力「約束を持つ `todo` のタスクへ `update_task` の `{ id, status: "done" }`」で値が残る）
- [ ] 遷移元が `todo` 以外でも、変更後が `todo` 以外になるステータス変更で約束が退役する（変異: 退役を遷移元が `todo` の更新に限る — 入力「約束を持つ `paused` のタスクへ `{ status: "in_progress" }`」で値が残る）
- [ ] 約束が退役した更新で記録される `task_update` イベントの `note` に「約束の退役（ステータス変更による）」と変更前の `committed_start_at` の値が含まれる（変異: 退役時に `note` を書かない — 入力「約束 `2026-09-14T20:00:00+09:00`・`todo` のタスクへ `{ status: "in_progress" }`」で `note` が `null` になる）
- [ ] ステータスを `todo` 以外へ変える更新で `committed_start_at` に新しい値も送ると、`committed_start_at` は `null` になる（変異: 送られた値を退役より優先する — 入力「約束を持つ `todo` のタスクへ `{ status: "in_progress", committed_start_at: "2026-09-14T21:00:00+09:00" }`」で `"2026-09-14T12:00:00.000Z"` が保存される）
...
- [ ] `POST /api/tasks` でステータスが `todo` 以外のタスクを作成すると、`committed_start_at` を送っても `null` で保存される（変異: 作成時に退役の規則を当てない — 入力「`{ title: "t", status: "in_progress", committed_start_at: "2026-09-14T20:00:00+09:00" }`」で値が保存される）
