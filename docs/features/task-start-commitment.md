...
- [ ] `PATCH /api/tasks/:id` と `POST /api/tasks` は入力の `committed_at` を無視する（入力「約束なしの `todo` のタスクへ `{ title: "改題", committed_at: "2000-01-01T00:00:00.000Z" }`」で `committed_at` は `null` のまま。変異: 入力の `committed_at` を保存する — 同じ入力で `"2000-01-01T00:00:00.000Z"` が保存される）
...
- [ ] `evaluateRules` が返す `commitment_missed` の `ruleKey` は `commitment_missed:{taskId}:{committed_start_at}:{committed_at}` である（入力: `taskId` 7・約束 14:00・`committed_at` 09:30 のとき `commitment_missed:7:{new Date(2026, 8, 14, 14, 0).toISOString()}:{new Date(2026, 8, 14, 9, 30).toISOString()}`）
