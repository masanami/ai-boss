...
// 夕会終了フックのテスト。保証 G-170-37 / G-170-38 が担保するのは
// 「初回遷移でのみ発火（再終了で再発火しない）」「要約保存と日報生成が
// 独立して走る」「生成失敗でも終了APIは200」「前提条件未達でも終了APIは200」。
// 朝会/随時で発火しないことも本ファイルで検証しているが、そのケースは
// `it.each` でテスト名が動的生成され台帳から参照できないため、保証ではなく
// GAP-30 として追跡している（台帳の HR-05 を参照）。#109 完了条件のテスト5〜9。
//
