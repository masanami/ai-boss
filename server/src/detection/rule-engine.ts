...
import { isWithinWorkingHours, toDateKey } from "./time-utils.js";
 ...
 *   ローカル暦日ごとに 1 回、commitment_missed は約束 1 件につき 1 回
...
export function evaluateRules(input: DetectionInput): FiringNotification[] {
  ...
  // ローカル暦日を足して 1 日 1 回だけ発火する（機能仕様
  // docs/features/working-hours-intervals.md 決定 9・10）。暦日を足すのは、
  // 日付を含まない rule_key だと一度鳴ったら二度と鳴らず、帯の中で積んだ履歴
  // とも衝突するため。
  function tryFireGated(
    ...
    if (withinWorkingHours) {
      ...
      fireOnce(ruleType, `${ruleKey}:${toDateKey(now)}`, taskId);
    }
  }
